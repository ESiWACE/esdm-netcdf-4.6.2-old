...
int main() {
  ...
  if (H5Dwrite(datasetid, H5T_NATIVE_HBOOL, H5S_ALL, H5S_ALL, H5P_DEFAULT,
      bool_out)
      < 0) ERR;
  if (H5Dclose(datasetid) < 0 || H5Sclose(spaceid) < 0 || H5Gclose(grpid) < 0 || H5Fclose(fileid) < 0)
    ERR;
...
#define TEMP_NAME "Temperature"
    ...
    /* Close up the shop. */
    if (H5Dclose(pres_dsid) < 0 || H5Dclose(temp_dsid) < 0 || H5Sclose(spaceid) < 0 || H5Gclose(grpid) < 0 || H5Fclose(fileid) < 0) ERR;
  }
...
#define DEFLATE_NAME "deflate"
  {
    ...
    if (H5Dwrite(datasetid, H5T_NATIVE_HBOOL, H5S_ALL, H5S_ALL, H5P_DEFAULT,
        bool_out)
        < 0) ERR;
    if (H5Dclose(datasetid) < 0 || H5Pclose(propid) < 0 || H5Sclose(spaceid) < 0 || H5Gclose(grpid) < 0 || H5Fclose(fileid) < 0)
      ERR;
    ...
    if ((filter = H5Pget_filter2(propid, 0, &flags, &cd_nelems, cd_values,
        ...
        < 0) ERR;
    if (filter != H5Z_FILTER_DEFLATE || cd_nelems != 1 || cd_values[0] != DEFLATE_LEVEL || strcmp(name, DEFLATE_NAME)) ERR;

    if (H5Dclose(datasetid) < 0 || H5Pclose(propid) < 0 || H5Gclose(grpid) < 0 || H5Fclose(fileid) < 0)
      ERR;
  }
    ...
    for (f = 0; f < num_filters; f++) {
      ...
      switch (filter) {
        ...
        case H5Z_FILTER_DEFLATE:
          found_deflate++;
          if (cd_nelems != 1 || cd_values[0] != DEFLATE_LEVEL || strcmp(name, DEFLATE_NAME)) ERR;
          break;
        default:
          break;
      }
...
#define BE_VAR_NAME "be_var"
  {
    ...
    if (H5Dwrite(be_did, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT,
        data)
        < 0) ERR;
    if (H5Dclose(native_did) < 0 || H5Dclose(le_did) < 0 || H5Dclose(be_did) < 0 || H5Sclose(spaceid) < 0 || H5Gclose(grpid) < 0 || H5Fclose(fileid) < 0)
      ERR;
    ...
    if (H5Dclose(native_did) < 0 || H5Dclose(le_did) < 0 || H5Dclose(be_did) < 0 || H5Gclose(grpid) < 0 || H5Fclose(fileid) < 0)
      ERR;
  }
