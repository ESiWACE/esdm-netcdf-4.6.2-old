...
int ncindexfind(NCindex *index, NC_OBJ *nco) {
  int i;
  NClist *list;
...
int ncindexadd(NCindex *ncindex, NC_OBJ *obj) {
...
/* Return 1 if ok, 0 otherwise.*/
int ncindexset(NCindex *ncindex, size_t i, NC_OBJ *obj) {
  if (ncindex == NULL) return 0;
 ...
 * Return 1 if ok, 0 otherwise.*/
int ncindexidel(NCindex *index, size_t i) {
...
int ncindexcount(NCindex *index) {
  int count = 0;
  int i;
...
int ncindexverify(NCindex *lm, int dump) {
  size_t i;
  NClist *l = lm->list;
  int nerrs = 0;
#ifndef NCNOHASH
