...
#  else
  ...
  for (i = 0; i < DIMSIZE; i++) {
    for (j = 0; j < STRLEN; j++) {
      if (string3_tst[i][j] != string3_data[i][j]) {
        report(i, "string3", __LINE__);
        break;
      }
    }
  }
#  endif
  ...
  for (i = 0; i < DIMSIZE; i++)
    printf("%s%hhu", COMMA, uint8_tst[i]);
  printf("};\n");
#else
  ...
  for (i = 0; i < DIMSIZE; i++)
    printf("%s%1.3f", COMMA, float32_tst[i]);
  printf("};\n");
#else
  ...
  for (i = 0; i < DIMSIZE; i++)
    printf("%s%1.3f", COMMA, float64_tst[i]);
  printf("};\n");
#else
...
#  define test                                       \
    ...
    printf("vdata =");                               \
    for (i = 0; i < DIMSIZE; i++) {                  \
      printf(" %llu", (unsigned long long)vdata[i]); \
    }                                                \
    printf("\n");                                    \
    for (i = 0; i < DIMSIZE; i++) {                  \
      if (v[i] != vdata[i]) {                        \
...
#  define ftest                                    \
    ...
    printf("\n");                                  \
    for (i = 0; i < DIMSIZE; i++) {                \
      if (fdiff((double)v[i], (double)vdata[i])) { \
        report(i, tag, line);                      \
        break;                                     \
      }                                            \
    }
