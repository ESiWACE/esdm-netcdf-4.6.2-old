...
#  else
  ...
  for (i = 0; i < DIMSIZE; i++) {
    for (j = 0; j < STRLEN; j++) {
      if (string3[i][j] != string3_data[i][j]) {
...
#  endif
  ...
  for (i = 0; i < DIMSIZE; i++)
    printf("%s%1.3f", COMMA, float32v[i]);
  printf("};\n");
#else
  ...
  for (i = 0; i < DIMSIZE; i++)
    printf("%s%1.3f", COMMA, float64v[i]);
  printf("};\n");
#else
