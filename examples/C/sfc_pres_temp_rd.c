...
int main() {
  ...
  /* In this case we know that there are 2 netCDF dimensions, 4
      ...
      dimension. */
  if (ndims_in != 2 || nvars_in != 4 || ngatts_in != 0 || unlimdimid_in != -1) return 2;
  ...
  for (lat = 0; lat < NLAT; lat++)
    for (lon = 0; lon < NLON; lon++)
      if (pres_in[lat][lon] != SAMPLE_PRESSURE + (lon * NLAT + lat) || temp_in[lat][lon] != SAMPLE_TEMP + .25 * (lon * NLAT + lat))
        return 2;
