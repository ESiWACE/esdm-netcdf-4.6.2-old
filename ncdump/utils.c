...
void check(int err, const char *file, const int line) {
  fprintf(stderr, "%s\n", nc_strerror(err));
...
int nc_inq_varid2(int ncid, const char *path0, int *varidp, int *grpidp) {
  int ret = NC_NOERR;
  int grpid, varid;
  char *v, *g, *prefix;
...
void idadd(idnode_t *vlist, int varid) {
 ...
 */
void freeidlist(idnode_t *idlist) {
  while (idlist) {
    idnode_t *vp = idlist->next;
    free(idlist);
    idlist = vp;
...
int grp_matches(int ncid, int nlgrps, char **lgrps, idnode_t *grpids) {
  int ig;
  size_t total = 0;
...
int nc_inq_gvarid(int grpid, const char *varname, int *varidp) {
 ...
 * exits. */
int missing_vars(int ncid, int nlvars, char **lvars) {
  int iv;
 ...
 * groups. */
int nc_get_giter(int grpid, /* start group id */
 ...
 */
int nc_next_giter(ncgiter_t *iterp, int *grpidp) {
  int stat = NC_NOERR;
  int numgrps;
  int *grpids;
  int i;
 ...
 * Release group iter.
 */
void nc_free_giter(ncgiter_t *iterp) {
  gs_free(iterp);
}
 ...
 */
int nc_inq_grps_full(int rootid, int *numgrps, int *grpids) {
  int stat = NC_NOERR;
