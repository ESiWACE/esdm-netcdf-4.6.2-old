 ...
 */
int chunkspec_parse(int igrp, const char *spec) {
  ...
  *p++ = '\0';
  ...
  while (*p) {
    ...
    if (sscanf(p, "%lu", &dimsize) != 1) {
      ret = NC_EINVAL;
      goto done;
    } /* Apparently not a valid dimension size */
    if (chunkspec->rank >= NC_MAX_VAR_DIMS) {
      ret = NC_EINVAL;
      goto done;
    } /* to many chunks */
