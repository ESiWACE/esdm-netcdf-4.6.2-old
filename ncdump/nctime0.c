...
void insert_bounds_info(int ncid, int varid, ncatt_t *attp) {
...
void get_timeinfo(int ncid1, int varid1, ncvar_t *vp) {
  ...
  if (nc_status == NC_NOERR && uatt.type == NC_CHAR) { /* TODO: NC_STRING? */
    ...
    if (cdParseRelunits(vp->timeinfo->calendar, units,
        &vp->timeinfo->unit, &vp->timeinfo->origin)
        != 0) {
