...
parsefilterspec(const char *optarg0, struct FilterSpec *spec) {
  ...
  if ((remainder == NULL) || (strncmp(remainder, "none", 4) == 0)) {
    spec->nofilter = 1;
    goto done;
  }
...
copy_var_specials(int igrp, int varid, int ogrp, int o_varid, int inkind, int outkind) {
  ...
  if (innc4 && outnc4) { /* handle checksum parameters */
    int fletcher32 = 0;
  ...
  if (innc4 && outnc4) { /* handle endianness */
    int endianness = 0;
 ...
 */
  ...
  if (option_kind == SAME_AS_INPUT) { /* default, kind not specified */
    ...
    if (inkind == NC_FORMAT_CLASSIC || inkind == NC_FORMAT_64BIT_OFFSET
      ...
      if (option_deflate_level > 0 || option_shuffle_vars == NC_SHUFFLE || listlength(option_chunkspecs) > 0) {
