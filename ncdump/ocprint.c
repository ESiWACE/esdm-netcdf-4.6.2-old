    ...
    switch (c) {
      ...
      case 'D': {
        ...
        if (c0 >= '0' && c0 <= '9') { /* debug level */
          ocopt.debug.debuglevel = atoi(optarg);
          break;
        } else
          switch (c0) {
      ...
      case 'p':
        ...
        if (strcasecmp(optarg, "das") == 0)
          ocopt.optdas = 1;
        else if (strcasecmp(optarg, "dds") == 0)
          ocopt.optdds = 1;
...
static void printstack(char *msg) {
  ...
  for (entry = stack, i = 0; i < stacknext; i++, entry++) {
    ...
    fprintf(stderr, "    [%d] (%s)", (int)i, name) for (j = 0; j < rank; j++)
