...
occompileatomic(OCstate *state, OCdata *data, XXDR *xxdrs) {
  ...
  if (!scalar) {
    ...
    if (!xxdr_uint(xxdrs, &xxdrcount)) {
      ocstat = OC_EXDR;
      goto fail;
    }
    if (xxdrcount != nelements) {
      ocstat = OC_EINVALCOORDS;
      goto fail;
    }
...
void ocdata_free(OCstate *state, OCdata *data) {
  if (data == NULL)
    return;
...
ocerrorstring(XXDR *xdrs) {
  ...
  MEMCHECK(data, 0);
  if (!xxdr_opaque(xdrs, data, avail)) {
    free(data);
    return 0;
  }
