...
#define CHECK(state, flag, value)                        \
  {                                                      \
    if (check(state, flag, (void *)value) != OC_NOERR) { \
      goto done;                                         \
    }                                                    \
  }
...
void oc_curl_debug(OCstate *state) {
