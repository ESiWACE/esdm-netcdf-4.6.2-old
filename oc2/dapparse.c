...
void dap_tagparse(DAPparsestate *state, int kind) {
  switch (kind) {
    case SCAN_DATASET:
    case SCAN_ERROR:
      break;
...
void dap_unrecognizedresponse(DAPparsestate *state) {
  ...
  for (i = 0; i < 4096; i++) {
    if (state->lexstate->input[i] == '\0') break;
  }
...
int dapsemanticerror(DAPparsestate *state, OCerror err, const char *msg) {
...
void dap_parse_error(DAPparsestate *state, const char *fmt, ...) {
