...
void ocdumpmemory(char *memory, size_t len, int xdrencoded, int level) {
...
void ocdd(OCstate *state, OCnode *root, int xdrencoded, int level) {
  char *mem;
  size_t len;
...
void ocdumpdata(OCstate *state, OCdata *data, NCbytes *buffer, int frominstance) {
  char tmp[1024];
...
void ocdumpdatapath(OCstate *state, OCdata *data, NCbytes *buffer) {
  int i;
  OCdata *path[1024];
  char tmp[1024];
  OCdata *pathdata;
