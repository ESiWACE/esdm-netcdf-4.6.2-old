...
void occomputesemantics(NClist *ocnodes) {
  unsigned int i, j;
...
void occomputefullnames(OCnode *root) {
  unsigned int i;
...
void occollectpathtonode(OCnode *node, NClist *path) {
  if (node == NULL) return;
...
void ocnodes_free(NClist *nodes) {
  unsigned int i, j;
...
ocddsdasmerge(OCstate *state, OCnode *dasroot, OCnode *ddsroot) {
  ...
  if (dasroot->tree == NULL || dasroot->tree->dxdclass != OCDAS) {
    stat = OCTHROW(OC_EINVAL);
    goto done;
  }
  if (ddsroot->tree == NULL || (ddsroot->tree->dxdclass != OCDDS && ddsroot->tree->dxdclass != OCDATADDS)) {
    stat = OCTHROW(OC_EINVAL);
    goto done;
  }
...
*/
void ocmarkcacheable(OCstate *state, OCnode *ddsroot) {
  int i, j;
#if 0
    int ok;
#endif
