...
int ocstrncmp(const char *s1, const char *s2, size_t len) {
  const char *p, *q;
  if (s1 == s2) return 0;
  if (s1 == NULL) return -1;
  if (s2 == NULL) return +1;
...
int ocfindbod(NCbytes *buffer, size_t *bodp, size_t *ddslenp) {
  unsigned int i;
  char *content;
...
void ocarrayindices(size_t index, size_t rank, size_t *sizes, size_t *indices) {
  int i;
  for (i = rank - 1; i >= 0; i--) {
...
int ocvalidateindices(size_t rank, size_t *sizes, size_t *indices) {
  int i;
  for (i = 0; i < rank; i++) {
...
*/
int occopycat(char *dst, size_t size, size_t n, ...) {
  va_list args;
  size_t avail = size - 1;
  int i;
...
ocmerge(const char **list1, const char **list2) {
  ...
  char **merge;
  const char **p;
  for (l1 = 0, p = list1; *p; p++) {
    l1++;
  }
  for (l2 = 0, p = list2; *p; p++) {
    l2++;
  }
