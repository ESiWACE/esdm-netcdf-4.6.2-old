...
var_create_dataset(NC_GRP_INFO_T *grp, NC_VAR_INFO_T *var, nc_bool_t write_dimid) {
  ...
  if (var->ndims) {
    ...
    /* If there are no unlimited dims, and no filters, and the user
       ...
       * better performance. */
    if (!var->shuffle && !var->deflate && !var->fletcher32 && (var->chunksizes == NULL || !var->chunksizes[0]) && !unlimdim)
  ...
  if (H5Pset_attr_creation_order(plistid, H5P_CRT_ORDER_TRACKED | H5P_CRT_ORDER_INDEXED) < 0)
    BAIL(NC_EHDFERR);
  ...
  if (!var->contiguous && var->chunk_cache_size)
    if (H5Pset_chunk_cache(access_plistid, var->chunk_cache_nelems,
        var->chunk_cache_size, var->chunk_cache_preemption)
        < 0)
      BAIL(NC_EHDFERR);
  ...
  if ((var->hdf_datasetid = H5Dcreate2(hdf5_grp->hdf_grpid, name_to_use, typeid,
       spaceid, H5P_DEFAULT, plistid, access_plistid))
      < 0)
    BAIL(NC_EHDFERR);
  var->created    = NC_TRUE;
...
create_group(NC_GRP_INFO_T *grp) {
  ...
  assert(grp && grp->format_grp_info && grp->parent && grp->parent->format_grp_info);
  ...
  if ((hdf5_grp->hdf_grpid = H5Gcreate2(parent_hdf5_grp->hdf_grpid, grp->hdr.name,
       H5P_DEFAULT, gcpl_id, H5P_DEFAULT))
      < 0)
    BAIL(NC_EHDFERR);
...
write_dim(NC_DIM_INFO_T *dim, NC_GRP_INFO_T *grp, nc_bool_t write_dimid) {
  ...
  if (!hdf5_dim->hdf_dimscaleid) {
    ...
    if (H5Pset_attr_creation_order(create_propid, H5P_CRT_ORDER_TRACKED | H5P_CRT_ORDER_INDEXED) < 0)
      BAIL(NC_EHDFERR);
        ...
        for (d = 0; d < var->ndims; d++) {
          ...
          for (g = grp; g && !finished; g = g->parent) {
            ...
            for (j = 0; j < ncindexsize(g->dim); j++) {
              ...
              if (var->dimscale_hdf5_objids[d].fileno[0] == hdf5_dim->hdf5_objid.fileno[0] && var->dimscale_hdf5_objids[d].objno[0] == hdf5_dim->hdf5_objid.objno[0] && var->dimscale_hdf5_objids[d].fileno[1] == hdf5_dim->hdf5_objid.fileno[1] && var->dimscale_hdf5_objids[d].objno[1] == hdf5_dim->hdf5_objid.objno[1]) {
      ...
      /* No dimscales for this var! Invent phony dimensions. */
        ...
        for (d = 0; d < var->ndims; d++) {
          ...
          for (match = -1, k = 0; k < ncindexsize(grp->dim); k++) {
            ...
            if ((dim->len == h5dimlen[d]) && ((h5dimlenmax[d] == H5S_UNLIMITED && dim->unlimited) || (h5dimlenmax[d] != H5S_UNLIMITED && !dim->unlimited))) {
              match = k;
              break;
            }
          }
