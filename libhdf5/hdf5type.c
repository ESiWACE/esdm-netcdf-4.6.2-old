...
nc_type typeid2, int *equalp) {
  ...
  /* If one is atomic, and the other user-defined, the types are not
    * equal. */
  if ((typeid1 <= NC_STRING && typeid2 > NC_STRING) || (typeid2 <= NC_STRING && typeid1 > NC_STRING)) {
    if (equalp) *equalp = 0;
    return NC_NOERR;
  }
 ...
 * @author Ed Hartnett
 */
int NC4_insert_compound(int ncid, nc_type typeid1, const char *name, size_t offset,
...
int ndims, const int *dim_sizesp) {
  ...
  LOG((2, "nc_insert_array_compound: ncid 0x%x, typeid %d name %s "
          "offset %d field_typeid %d ndims %d",
  ncid, typeid1,
 ...
 * @author Ed Hartnett
 */
int NC4_def_opaque(int ncid, size_t datum_size, const char *name,
nc_type *typeidp) {
 ...
 * @author Ed Hartnett
 */
int NC4_def_vlen(int ncid, const char *name, nc_type base_typeid,
nc_type *typeidp) {
 ...
 * @author Ed Hartnett
 */
int NC4_def_enum(int ncid, nc_type base_typeid, const char *name,
nc_type *typeidp) {
 ...
 * @author Ed Hartnett
 */
int NC4_insert_enum(int ncid, nc_type typeid1, const char *identifier,
const void *value) {
  NC_GRP_INFO_T *grp;
  NC_TYPE_INFO_T *type;
  char norm_name[NC_MAX_NAME + 1];
