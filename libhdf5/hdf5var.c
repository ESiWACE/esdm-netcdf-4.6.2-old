...
nc4_find_default_chunksizes2(NC_GRP_INFO_T *grp, NC_VAR_INFO_T *var) {
  ...
  if (var->ndims == 1 && num_unlim == 1) {
    ...
    LOG((4, "%s: name %s dim %d DEFAULT_CHUNK_SIZE %d num_values %f type_size %d "
            "chunksize %ld",
    __func__, var->hdr.name, d, DEFAULT_CHUNK_SIZE, num_values, type_size, var->chunksizes[0]));
  }
  if (var->ndims > 1 && var->ndims == num_unlim) { /* all dims unlimited */
    ...
    for (d = 0; d < var->ndims; d++) {
      ...
      LOG((4, "%s: name %s dim %d DEFAULT_CHUNK_SIZE %d num_values %f type_size %d "
              "chunksize %ld",
      __func__, var->hdr.name, d, DEFAULT_CHUNK_SIZE, num_values, type_size, var->chunksizes[d]));
    }
  }
  ...
  for (d = 0; d < var->ndims; d++)
    if (!var->chunksizes[d]) {
      suggested_size = (pow((double)DEFAULT_CHUNK_SIZE / (num_values * type_size),
                        1.0 / (double)(var->ndims - num_unlim))
                        * var->dim[d]->len
                        - .5);
 ...
 */
int NC4_def_var(int ncid, const char *name, nc_type xtype,
int ndims, const int *dimidsp, int *varidp) {
  ...
  if (dim && (!var->ndims || dimidsp[0] != dim->hdr.id)) {
    ...
    if (strlen(norm_name) + strlen(NON_COORD_PREPEND) > NC_MAX_NAME)
      BAIL(NC_EMAXNAME);
    if (!(var->hdf5_name = malloc((strlen(NON_COORD_PREPEND) + strlen(norm_name) + 1) * sizeof(char))))
      BAIL(NC_ENOMEM);
...
const void *fill_value, int *endianness) {
  ...
  assert((deflate && deflate_level && shuffle) || (!deflate && !deflate_level && !shuffle));
  ...
  if (deflate) {
    if (*deflate)
      if (*deflate_level < NC_MIN_DEFLATE_LEVEL || *deflate_level > NC_MAX_DEFLATE_LEVEL)
        return NC_EINVAL;
 ...
 * @author Ed Hartnett, Dennis Heimbigner
 */
int NC4_def_var_deflate(int ncid, int varid, int shuffle, int deflate,
int deflate_level) {
 ...
 */
int NC4_def_var_filter(int ncid, int varid, unsigned int id, size_t nparams,
...
int NC4_rename_var(int ncid, int varid, const char *name) {
  ...
  if (!(h5->flags & NC_INDEF) && strlen(name) > strlen(var->hdr.name) && (h5->cmode & NC_CLASSIC_MODEL))
 ...
 * @author Ed Hartnett, Dennis Heimbigner
 */
int NC4_put_vara(int ncid, int varid, const size_t *startp,
 ...
 * @author Ed Hartnett, Dennis Heimbigner
 */
int NC4_get_vara(int ncid, int varid, const size_t *startp,
...
check_for_vara(nc_type *mem_nc_type, NC_VAR_INFO_T *var, NC_FILE_INFO_T *h5) {
  ...
  if (var->type_info->hdr.id != *mem_nc_type && (var->type_info->hdr.id == NC_CHAR || *mem_nc_type == NC_CHAR))
    return NC_ECHAR;
...
set_par_access(NC_FILE_INFO_T *h5, NC_VAR_INFO_T *var, hid_t xfer_plistid) {
  ...
  if (h5->parallel) {
    ...
    hdf5_xfer_mode = (var->parallel_access != NC_INDEPENDENT) ? H5FD_MPIO_COLLECTIVE : H5FD_MPIO_INDEPENDENT;
 ...
 * @author Ed Hartnett, Dennis Heimbigner
 */
int NC4_put_vars(int ncid, int varid, const size_t *startp, const size_t *countp,
const ptrdiff_t *stridep, const void *data, nc_type mem_nc_type) {
  ...
  for (d2 = 0; d2 < var->ndims; d2++) {
    ...
    if (!dim->unlimited) {
      ...
      if (start[d2] > (hssize_t)fdims[d2] || (start[d2] == (hssize_t)fdims[d2] && count[d2] > 0))
  ...
  /* Are we going to convert any data? (No converting of compound or
    * opaque types.) */
  if (mem_nc_type != var->type_info->hdr.id && mem_nc_type != NC_COMPOUND && mem_nc_type != NC_OPAQUE) {
  ...
  if (H5Dwrite(var->hdf_datasetid, var->type_info->hdf_typeid,
      mem_spaceid, file_spaceid, xfer_plistid, bufr)
      < 0)
    BAIL(NC_EHDFERR);
  ...
  /* For strict netcdf-3 rules, ignore erange errors between UBYTE
    * and BYTE types. */
  if ((h5->cmode & NC_CLASSIC_MODEL) && (var->type_info->hdr.id == NC_UBYTE || var->type_info->hdr.id == NC_BYTE) && (mem_nc_type == NC_UBYTE || mem_nc_type == NC_BYTE) && range_error)
    range_error = 0;
 ...
 * @author Ed Hartnett, Dennis Heimbigner
 */
int NC4_get_vars(int ncid, int varid, const size_t *startp, const size_t *countp,
const ptrdiff_t *stridep, void *data, nc_type mem_nc_type) {
...
#ifdef RELAX_COORD_BOUND
      /* Allow start to equal dim size if count is zero. */
      if (start[d2] > (hssize_t)ulen || (start[d2] == (hssize_t)ulen && count[d2] > 0))
        BAIL_QUIET(NC_EINVALCOORDS);
#else
  ...
  if (!no_read) {
    ...
    if (var->type_info->nc_type_class == NC_STRING && H5Tget_size(var->type_info->hdf_typeid) > 1 && !H5Tis_variable_str(var->type_info->hdf_typeid)) {
      hsize_t fstring_len;
