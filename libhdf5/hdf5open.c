...
#  endif
  ...
  if (nc4_info->mem.inmemory) {
    ...
    if (retval)
      BAIL(NC_EHDFERR);
  } else if (nc4_info->mem.diskless) { /* Process  NC_DISKLESS */
 ...
 * @author Ed Hartnett
 */
int NC4_open(const char *path, int mode, int basepe, size_t *chunksizehintp,
void *parameters, NC_Dispatch *dispatch, NC *nc_file) {
  assert(nc_file && path && dispatch && nc_file && nc_file->model == NC_FORMATX_NC4);
...
size_t ndims, NC_DIM_INFO_T *dim) {
  ...
  if (strlen(obj_name) > strlen(NON_COORD_PREPEND) && !strncmp(obj_name, NON_COORD_PREPEND, strlen(NON_COORD_PREPEND))) {
...
int nc4_read_atts(NC_GRP_INFO_T *grp, NC_VAR_INFO_T *var) {
  ...
  locid = var ? var->hdf_datasetid : ((NC_HDF5_GRP_INFO_T *)(grp->format_grp_info))->hdf_grpid;
...
nc4_rec_read_metadata(NC_GRP_INFO_T *grp) {
  ...
  if (H5Literate(hdf5_grp->hdf_grpid, iter_index, H5_ITER_INC, &idx,
      nc4_rec_read_metadata_cb, (void *)&udata)
      < 0)
    BAIL(NC_EHDFERR);
