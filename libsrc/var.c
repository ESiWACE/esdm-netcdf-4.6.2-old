...
#ifdef MALLOCHACK
  const size_t sz = M_RNDUP(sizeof(NC_var)) + o1 + o2 + ndims * sizeof(off_t);
#else  /*!MALLOCHACK*/
...
int NC_var_shape(NC_var *varp, const NC_dimarray *dims) {
  ...
  /*
	 ...
	 */
  for (ip = varp->dimids, op = varp->shape; ip < &varp->dimids[varp->ndims]; ip++, op++) {
 ...
 */
int NC_check_vlen(NC_var *varp, long long vlen_max) {
  int ii;
...
int NC3_def_var(int ncid, const char *name, nc_type type,
...
int NC3_rename_var(int ncid, int varid, const char *unewname) {
  int status = NC_NOERR;
  NC *nc;
  NC3_INFO *ncp;
  uintptr_t intdata;
