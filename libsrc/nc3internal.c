 ...
 */
int nc3_cktype(int mode, nc_type type) {
#ifdef ENABLE_CDF5
...
fprintf(stderr, "    VAR %d %s: %ld\n", ii, (*vpp)->name->cp, (long)index);
  ...
  if (ncp->begin_rec < index + v_minfree || ncp->begin_rec != D_RNDUP(ncp->begin_rec, r_align)) {
 ...
 */
int NC_check_vlens(NC3_INFO *ncp) {
  NC_var **vpp;
 ...
 */
int NC_check_voffs(NC3_INFO *ncp) {
  size_t i;
  off_t prev_off;
  NC_var *varp;
...
int NC3_open(const char *path, int ioflags, int basepe, size_t *chunksizehintp,
...
int NC3_close(int ncid, void *params) {
  int status = NC_NOERR;
  NC *nc;
  NC3_INFO *nc3;
...
int NC3_inq_unlimdim(int ncid, int *xtendimp) {
  int status;
  NC *nc;
  NC3_INFO *nc3;
...
void NC_set_numrecs(NC *nc3, size_t nrecs) {
...
/*ARGSUSED*/
int NC3_set_base_pe(int ncid, int pe) {
#if _CRAYMPP && defined(LOCKNUMREC)
  int status;
  NC *nc;
  NC3_INFO *nc3;
...
/*ARGSUSED*/
int NC3_inq_base_pe(int ncid, int *pe) {
#if _CRAYMPP && defined(LOCKNUMREC)
  int status;
  NC *nc;
  NC3_INFO *nc3;
 ...
 */
int NC3_inq_format(int ncid, int *formatp) {
  int status;
  NC *nc;
  NC3_INFO *nc3;
 ...
 */
int NC3_inq_format_extended(int ncid, int *formatp, int *modep) {
  int status;
  NC *nc;
 ...
 * \author Ed Hartnett
 */
int NC3_inq_type(int ncid, nc_type typeid, char *name, size_t *size) {
  NC *ncp;
...
int NC3_inq_default_fill_value(int xtype, void *fillp) {
...
int NC3_inq_var_fill(const NC_var *varp, void *fill_value) {
  NC_attr **attrpp = NULL;
