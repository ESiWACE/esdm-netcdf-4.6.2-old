...
xdr_NC_fill()
 */
int fill_NC_var(NC3_INFO *ncp, const NC_var *varp, long long varsize, size_t recno) {
...
const size_t *start, const size_t *edges) {
  ...
  if (IS_RECVAR(varp)) {
#ifdef RELAX_COORD_BOUND
    if (NC_readonly(ncp) && (start[0] == NC_get_numrecs(ncp) && edges[0] > 0))
      return (NC_EINVALCOORDS);
#endif
    start++;
    edges++;
...
#endif
    /* cast needed for braindead systems with signed size_t */
    if ((unsigned long)*edges > *shp || (unsigned long)*start + (unsigned long)*edges > *shp) {
      return (NC_EEDGE);
    }
  }
  return NC_NOERR;
...
int NC3_get_vara(int ncid, int varid,
...
int NC3_put_vara(int ncid, int varid,
const size_t *start, const size_t *edges0,
const void *value0,
nc_type memtype) {
  int status = NC_NOERR;
