...
ncmpix_len_nctype(nc_type type) {
  switch (type) {
    ...
    default:
      fprintf(stderr, "ncmpix_len_nctype bad type %d\n", type);
      assert(0);
  }
  return 0;
}
...
v1h_put_NC_var(v1hs *psp, const NC_var *varp) {
  ...
  /* write vsize to header.
     ...
     */
  vsize = varp->len;
  if (varp->len > 4294967292UL && (psp->version == NC_FORMAT_CLASSIC || psp->version == NC_FORMAT_64BIT_OFFSET))
...
NC_computeshapes(NC3_INFO *ncp) {
  ...
  if (ncp->begin_var <= 0 || ncp->xsz > (size_t)ncp->begin_var || ncp->begin_rec <= 0 || ncp->begin_var > ncp->begin_rec)
    return (NC_ENOTNC); /* not a netCDF file or corrupted */
...
int ncx_put_NC(const NC3_INFO *ncp, void **xpp, off_t offset, size_t extent) {
  int status = NC_NOERR;
