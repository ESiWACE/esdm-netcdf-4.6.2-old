...
void nclog(int tag, const char *fmt, ...) {
  va_list args;
  char *prefix;
...
void ncvlog(int tag, const char *fmt, va_list ap) {
  char *prefix;
...
void nclogtext(int tag, const char *text) {
...
void nclogtextn(int tag, const char *text, size_t count) {
  if (!nclogging || nclogstream == NULL) return;
...
void nclogsettags(char **tagset, char *dfalt) {
  nctagdfalt = dfalt;
  if (tagset == NULL) {
    nctagsize = 0;
  } else {
