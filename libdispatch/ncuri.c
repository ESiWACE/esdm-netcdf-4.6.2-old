...
#define lshift(buf, buflen) \
  { memmove(buf, buf + 1, buflen + 1); }
#define rshift(buf, buflen) \
  { memmove(buf + 1, buf, buflen + 1); }

/* Allowable character sets for encode */
static char *pathallow = "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ!#$&'()*+,-./:;=?@_~";

static char *queryallow = "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ!#$&'()*+,-./:;=?@_~";

/* user+pwd allow = path allow - "@:" */
static char *userpwdallow = "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ!$&'()*+,-.;=_~?#/";
...
int ncuriparse(const char *uri0, NCURI **durip) {
  int ret = NCU_OK;
  NCURI tmp;
  char *p;
  char *q;
...
int ncurisetprotocol(NCURI *duri, const char *protocol) {
  nullfree(duri->protocol);
...
/* Replace the query */
int ncurisetquery(NCURI *duri, const char *query) {
  int ret = NCU_OK;
  freestringvec(duri->querylist);
  nullfree(duri->query);
  duri->query = NULL;
