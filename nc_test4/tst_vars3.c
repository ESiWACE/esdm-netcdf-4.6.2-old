...
#define D2_NAME "D2"
    ...
    if (strcmp(name_in, V1_NAME) || xtype_in != NC_DOUBLE || ndims != 1 || natts != 0 || dimids_in[0] != y_dimid) ERR;
    if (nc_inq_var(ncid, 1, name_in, &xtype_in, &ndims, dimids_in, &natts)) ERR;
    if (strcmp(name_in, D1_NAME) || xtype_in != NC_DOUBLE || ndims != 1 || natts != 0 || dimids_in[0] != x_dimid) ERR;
...
#define TEXT_LEN 15
    ...
    if (strcmp(var_name_in, VAR_NAME1) || xtype_in <= NC_STRING || ndims_in != 0 || natts_in != 0) ERR;
    if (nc_close(ncid)) ERR;
  }
  SUMMARIZE_ERR;
...
#define D1_LEN 99
    ...
    if (nc_inq_var(ncid, 0, var_name_in, &xtype_in, &ndims_in, dimids_in, &natts_in)) ERR;
    if (strcmp(var_name_in, VAR_NAME2) || xtype_in != NC_UINT64 || ndims_in != 2 || natts_in != 0) ERR;
...
#  define NC_SZIP_EC_OPTION_MASK 4  /**< @internal SZIP EC option mask. */
    ...
    if (nc_inq_var(ncid, 0, var_name_in, &xtype_in, &ndims_in, &dimids_in, &natts_in)) ERR;
    if (strcmp(var_name_in, V_SMALL) || xtype_in != NC_INT64 || ndims_in != 1 || natts_in != 0) ERR;
