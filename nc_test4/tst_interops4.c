...
int read_vars(int ncid) {
  ...
  for (v = 0; v < NUM_VARS; v++) {
    ...
    sprintf(var_name, "var_%d", v);
    if (strcmp(var_name, var_name_in) || xtype_in != NC_INT || ndims_in != NUM_DIMS || natts_in != NUM_ATTS) ERR_RET;
    read_atts(ncid, v);
...
int main(int argc, char **argv) {
  ...
  printf("*** testing with file with lots of atts...");
  {
    ...
    if (ndims_in != NUM_DIMS || nvars_in != NUM_VARS || natts_in != NUM_ATTS || unlimdim_in != -1) ERR;
