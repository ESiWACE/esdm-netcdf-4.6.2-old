...
int main(int argc, char **argv) {
  ...
  printf("*** testing compound variable create from packed struct...");
  {
    ...
    if (field_ndims) ERR;
    if (strcmp(name_in, I_NAME) || offset != NC_COMPOUND_OFFSET(struct s1, i) || (field_xtype != NC_SHORT || field_ndims != 0)) ERR;
    if (nc_inq_compound_field(ncid, xtype, 1, name_in, &offset, &field_xtype, &field_ndims,
        field_sizes)) ERR;
    if (strcmp(name_in, J_NAME) || offset != NC_COMPOUND_OFFSET(struct s1, j) || field_xtype != NC_INT64) ERR;
...
#define ATT_LEN 1
    ...
    if (a1_att_in[0].s1.x != a1_att[0].s1.x || a1_att_in[0].s1.y != a1_att[0].s1.y) ERR;
