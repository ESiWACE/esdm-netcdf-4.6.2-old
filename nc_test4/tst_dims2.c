...
#define NUM_NUTS 5
    ...
    if (strcmp(name_in, CRAZY) || xtype_in != xtype || ndims_in != NDIMS1 || natts_in != 0 || dimid_in != nuts_dimid) ERR;
...
#define TL 15
    ...
    if (strcmp(name_in, "time") || xtype_in != NC_CHAR || ndims_in != NDIMS2 || natts_in != 0) ERR;
    for (i = 0; i < NDIMS2; i++)
...
#define NDIMS 2
    ...
    if (nc_inq_var(ncid, 0, name_in, &xtype_in, &ndims_in, dimids_in, &natts_in)) ERR;
    if (strcmp(name_in, TIME) || xtype_in != xtype || ndims_in != NDIMS || natts_in != 0) ERR;
    for (i = 0; i < NDIMS; i++)
