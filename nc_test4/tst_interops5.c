...
#define NDIMS 2
    ...
    if ((fileId = H5Fcreate(FILE_NAME, H5F_ACC_TRUNC,
         H5Pcreate(H5P_FILE_CREATE), fapl))
        < 0) ERR;
...

    {
      ...
      if (nc_inq_var(grpid, 0, name_in, &xtype_in, &ndims_in, dimid_in,
          &natts_in)) ERR;
      if (strcmp(name_in, VAR_NAME) || xtype_in != NC_SHORT || ndims_in != NDIMS || dimid_in[0] != 0 || dimid_in[1] != 1 || natts_in != 0) ERR;
...
#  define BATTLE_RECORD "Battle_Record"
    ...
    if (nc_inq_var(nc_grpid, 0, name_in, &xtype_in, &ndims_in, dimid_in,
        &natts_in)) ERR;
    if (strcmp(name_in, BATTLE_RECORD) || xtype_in != NC_INT || ndims_in != 1 || dimid_in[0] != 0 || natts_in != 0) ERR;
