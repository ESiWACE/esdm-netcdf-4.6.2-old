...
#define VAR_NAME3 "Bosworth_Field"
    ...
    if (strcmp(name_in, VAR_NAME1) || xtype_in != NC_INT64 || ndims_in != NDIMS_IN_VAR || dimid1_in != dimid1 || natts != 0) ERR;
    if (nc_inq_var(grpid_in, varid2, name_in, &xtype_in, &ndims_in, &dimid1_in, &natts)) ERR;
    if (strcmp(name_in, VAR_NAME2) || xtype_in != NC_INT64 || ndims_in != NDIMS_IN_VAR || dimid1_in != dimid1 || natts != 0) ERR;
    if (nc_inq_var(grpid_in, varid3, name_in, &xtype_in, &ndims_in, &dimid1_in, &natts)) ERR;
    if (strcmp(name_in, VAR_NAME3) || xtype_in != NC_INT64 || ndims_in != NDIMS_IN_VAR || dimid1_in != dimid2 || natts != 0) ERR;
  ...
  printf("*** testing groups and vars...");
  {
    ...
    if (nc_inq_var(dynasty, 0, name_in, &xtype_in, &ndims_in, dimids_in,
        &natts_in)) ERR;
    if (strcmp(name_in, VAR1_NAME) || xtype_in != NC_INT || ndims_in != 1 || dimids_in[0] != 0 || natts_in != 0) ERR;
    if (nc_close(ncid)) ERR;
  }
  SUMMARIZE_ERR;
  ...
  printf("*** testing groups and vars...");
  {
    ...
    for (i = 0; i < 5; i++) {
      ...
      if (nc_inq_var(grpids_in[i], 0, name_in, &xtype_in, &ndims_in, dimids_in,
          &natts_in)) ERR;
      if (strcmp(name_in, VAR1_NAME) || xtype_in != NC_INT || ndims_in != 1 || dimids_in[0] != i || natts_in != 0) ERR;
    ...
    for (i = 0; i < 5; i++) {
	  ...
	  * dimid is not i. */
      ...
      if (nc_inq_var(grpids_in[i], 0, name_in, &xtype_in, &ndims_in, dimids_in,
          &natts_in)) ERR;
      if (strcmp(name_in, VAR1_NAME) || xtype_in != NC_INT || ndims_in != 1 || natts_in != 0) ERR;
      if (nc_get_var_int(grpids_in[i], 0, data_in)) ERR;
      for (j = 0; j < DIM1_LEN; j++)
        if (data_in[j] != data_out[j]) ERR;
    }
  ...
  printf("*** testing very simple groups and dimension scoping...");
  {
    ...
    if (nc_inq_var(henry_vii_id, 0, name_in, &xtype_in, &ndims_in, dimids_in,
        &natts_in)) ERR;
    if (strcmp(name_in, VAR1_NAME) || xtype_in != NC_INT || ndims_in != 1 || dimids_in[0] != dimid || natts_in != 0) ERR;
...
#define NUM_GRPS 5
  ...
  printf("*** testing more groups and dimension scoping...");
  {
    ...
    if (strcmp(name_in, VAR1_NAME) || xtype_in != NC_UINT64 || ndims_in != 2 || dimids_in[0] != year_did || dimids_in[1] != kingdom_did || natts_in != 0) ERR;
