...
size_t *chunk_size) {
  ...
  for (t = 0; t < NUM_TRIES; t++) {
    ...
    endtime = MPI_Wtime();
    if (!mpi_rank) {
      bandwidth += ((sizeof(float) * DIMSIZE1 * DIMSIZE2 * NUMVARS) / ((endtime - starttime) * 1024 * 1024)) / NUM_TRIES;
...
MPI_Info info, int mpi_size, int mpi_rank, size_t *chunk_size) {
  ...
  for (t = 0; t < NUM_TRIES; t++) {
    ...
    if (!mpi_rank) {
      write_time += (endtime - starttime) / NUM_TRIES;
      bandwidth += (sizeof(float) * TIMELEN * DIMSIZE1 * DIMSIZE2 * DIMSIZE3 * NUMVARS) / ((endtime - starttime) * 1024 * 1024 * NUM_TRIES);
    }
  }
  free(udata);
  if (!mpi_rank) {
