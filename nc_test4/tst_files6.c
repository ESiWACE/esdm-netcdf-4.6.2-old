...
#define VAR_NAME2 "var2"
    ...
    if (strcmp(name_in, VAR_NAME1) || xtype_in != NC_FLOAT || ndims_in != 1 || dimid_in != 0 || natts_in != 0) ERR;
    if (nc_inq_var(ncid, 1, name_in, &xtype_in, &ndims_in,
        &dimid_in, &natts_in)) ERR;
    if (strcmp(name_in, VAR_NAME2) || xtype_in != NC_FLOAT || ndims_in != 1 || dimid_in != 0 || natts_in != 0) ERR;
    if (nc_inq_var(ncid, 2, name_in, &xtype_in, &ndims_in,
        &dimid_in, &natts_in)) ERR;
    if (strcmp(name_in, DIM_NAME) || xtype_in != NC_FLOAT || ndims_in != 1 || dimid_in != 0 || natts_in != 0) ERR;
    if (nc_close(ncid)) ERR;
  }
  SUMMARIZE_ERR;
