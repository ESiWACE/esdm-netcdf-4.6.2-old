...
int main(int argc, char **argv) {
  ...
  printf("*** Testing NC_BYTE conversions...");
  {
    ...
    if (strcmp(var_name, VAR_NAME) || natts != 0 || ndims != 0 || var_type != NC_BYTE) ERR;
  ...
  printf("*** Testing NC_USHORT conversions...");
  {
    ...
    if (nc_open(FILE_NAME, NC_NOWRITE, &ncid)) ERR;
    if (nc_inq_var(ncid, 0, var_name, &var_type, &ndims, NULL, &natts)) ERR;
    if (strcmp(var_name, VAR_NAME) || natts != 0 || ndims != 1 || var_type != NC_USHORT) ERR;
    coord[0] = 0;
...
#define X_INT_MAX 2147483647 /* from libsrc4/nc4internal.h */
    ...
    if (nc_open(FILE_NAME, NC_NOWRITE, &ncid)) ERR;
    if (nc_inq_var(ncid, 0, var_name, &var_type, &ndims, NULL, &natts)) ERR;
    if (strcmp(var_name, VAR_NAME) || natts != 0 || ndims != 0 || var_type != NC_INT) ERR;
