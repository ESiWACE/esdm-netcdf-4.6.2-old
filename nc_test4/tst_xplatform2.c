...
int check_file_1(int ncid, nc_vlen_t *data_out) {
  ...
  if (strcmp(name_in, X_NAME) || offset_in != 0 || field_type_in != NC_FLOAT || ndims_in) ERR;
  if (nc_inq_compound_field(ncid, typeids_in[0], 1, name_in, &offset_in,
      &field_type_in, &ndims_in, NULL)) ERR;
  if (strcmp(name_in, Y_NAME) || offset_in != NC_COMPOUND_OFFSET(struct s1, y) || field_type_in != NC_DOUBLE || ndims_in) ERR;

  /* How does the vlen type look? */
  if (nc_inq_vlen(ncid, typeids_in[1], name_in, &size_in, &base_nc_type_in)) ERR;
  if (strcmp(name_in, VLEN_NAME) || size_in != sizeof(nc_vlen_t) || base_nc_type_in != typeids_in[0]) ERR;
  ...
  for (i = 0; i < DIM1_LEN; i++) {
    ...
    for (j = 0; j < data_in[i].len; j++)
      if (((struct s1 *)data_in[i].p)->x != ((struct s1 *)data_out[i].p)->x || ((struct s1 *)data_in[i].p)->y != ((struct s1 *)data_out[i].p)->y) ERR_RET;
  }
...
int check_file_2(int ncid, struct s2 *data_out) {
  ...
  if (strcmp(name_in, S2_TYPE_NAME) || size_in != sizeof(struct s2) || nfields_in != 1) ERR;
  if (nc_inq_compound_field(ncid, typeids_in[1], 0, name_in, &offset_in, &field_type_in,
      &ndims_in, field_dims_in)) ERR;
  if (strcmp(name_in, S1_NAME) || offset_in != NC_COMPOUND_OFFSET(struct s2, data) || field_type_in != typeids_in[0] || ndims_in != 1 || field_dims_in[0] != NUM_S1) ERR;
  ...
  for (i = 0; i < DIM2_LEN; i++)
    for (j = 0; j < NUM_S1; j++)
      if (data_out[i].data[j].x != data_in[i].data[j].x || data_out[i].data[j].y != data_in[i].data[j].y) ERR;
...
int check_file_3(int ncid, struct s3 *data_out) {
  ...
  if (strcmp(name_in, S3_TYPE_NAME) || size_in != sizeof(struct s3) || nfields_in != 1) ERR;
  if (nc_inq_compound_field(ncid, typeids_in[2], 0, name_in, &offset_in, &field_type_in,
      &ndims_in, field_dims_in)) ERR;
  if (strcmp(name_in, VL_NAME) || offset_in != NC_COMPOUND_OFFSET(struct s3, data) || field_type_in != typeids_in[1] || ndims_in != 1 || field_dims_in[0] != NUM_VL) ERR;
  ...
  for (i = 0; i < DIM3_LEN; i++)
    for (j = 0; j < NUM_VL; j++) {
      ...
      for (k = 0; k < data_out[i].data[j].len; k++)
        if (((struct s1 *)data_in[i].data[j].p)[k].x != ((struct s1 *)data_out[i].data[j].p)[k].x || ((struct s1 *)data_in[i].data[j].p)[k].y != ((struct s1 *)data_out[i].data[j].p)[k].y) ERR;
    }
  ...
  printf("*** Opening tst_xplatform2_3.nc...");
  {
    ...
    for (i = 0; i < num_obj; i++) {
      ...
      if (H5Lget_name_by_idx(grpid, ".", H5_INDEX_NAME, H5_ITER_INC, i,
          obj_name, NC_MAX_NAME + 1, H5P_DEFAULT)
          < 0) ERR_RET;
