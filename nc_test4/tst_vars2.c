...
#define DIM4_LEN 10
  {
    ...
    if (ndims != NDIMS4 || nvars != 1 || natts != 0 || unlimdimid != -1) ERR;
    ...
    if (nc_inq_var(ncid, 0, name_in, &xtype_in, &ndims,
        dimids_in, &natts)) ERR;
    if (strcmp(name_in, VAR_NAME4) || xtype_in != NC_INT || ndims != 1 || natts != 0 || dimids_in[0] != 0) ERR;
...
#define NVAR4 5
    ...
    /* Check stuff. */
    if (nc_inq(ncid, &ndims, &nvars, &natts, &unlimdimid)) ERR;
    if (ndims != NDIMS5 || nvars != 1 || natts != 0 || unlimdimid != -1) ERR;
    ...
    if (strcmp(name_in, VAR_NAME5) || xtype_in != NC_INT || ndims != 1 || natts != 0 || dimids_in[0] != 0) ERR;
    ...
    if (cache_size_in != CACHE_SIZE || cache_nelems_in != CACHE_NELEMS || cache_preemption_in != CACHE_PREEMPTION) ERR;
    ...
    /* Check stuff. */
    if (nc_inq(ncid, &ndims, &nvars, &natts, &unlimdimid)) ERR;
    if (ndims != NDIMS5 || nvars != NVAR4 || natts != 0 || unlimdimid != -1) ERR;
    if (nc_inq_varids(ncid, &nvars, varids_in4)) ERR;
    if (nvars != NVAR4) ERR;
    ...
    if (nc_get_var_chunk_cache_ints(ncid, varid, &cache_size_int_in, &cache_nelems_int_in,
        &cache_preemption_int_in)) ERR;
    if (cache_size_int_in != CACHE_SIZE2 / MEGABYTE || cache_nelems_int_in != CACHE_NELEMS2 || cache_preemption_int_in != (int)(CACHE_PREEMPTION2 * 100)) ERR;
    ...
    if (nc_get_var_chunk_cache_ints(ncid, varid, &cache_size_int_in, &cache_nelems_int_in,
        &cache_preemption_int_in)) ERR;
    if (cache_size_int_in != CACHE_SIZE / MEGABYTE || cache_nelems_int_in != cache_nelems_int_default || cache_preemption_int_in != cache_preemption_int_default) ERR;
    /* Now set the nelems only. */
    if (nc_set_var_chunk_cache_ints(ncid, varid, -CACHE_SIZE / MEGABYTE, CACHE_NELEMS,
        -(int)(CACHE_PREEMPTION2 * 100))) ERR;
    if (nc_get_var_chunk_cache_ints(ncid, varid, &cache_size_int_in, &cache_nelems_int_in,
        &cache_preemption_int_in)) ERR;
    if (cache_size_int_in != cache_size_int_default || cache_nelems_int_in != CACHE_NELEMS || cache_preemption_int_in != cache_preemption_int_default) ERR;
    /* Now set the preemption only. */
    if (nc_set_var_chunk_cache_ints(ncid, varid, -CACHE_SIZE / MEGABYTE, -CACHE_NELEMS,
        (int)(CACHE_PREEMPTION2 * 100))) ERR;
    if (nc_get_var_chunk_cache_ints(ncid, varid, &cache_size_int_in, &cache_nelems_int_in,
        &cache_preemption_int_in)) ERR;
    if (cache_size_int_in != cache_size_int_default || cache_nelems_int_in != cache_nelems_int_default || cache_preemption_int_in != (int)(CACHE_PREEMPTION2 * 100)) ERR;
...
#define DIM6_LEN 100
    ...
    /* Check stuff. */
    if (nc_inq(ncid, &ndims, &nvars, &natts, &unlimdimid)) ERR;
    if (ndims != NDIMS1 || nvars != 1 || natts != 0 || unlimdimid != -1) ERR;
    if (nc_inq_varids(ncid, &nvars, varids_in)) ERR;
    if (nvars != 1) ERR;
    if (varids_in[0] != 0) ERR;
    if (nc_inq_var(ncid, 0, name_in, &xtype_in, &ndims, dimids_in, &natts)) ERR;
    if (strcmp(name_in, VAR_NAME6) || xtype_in != NC_INT || ndims != 1 || natts != 0 || dimids_in[0] != 0) ERR;
    if (nc_inq_var_chunking(ncid, 0, &storage_in, chunksize_in)) ERR;
    if (storage_in != NC_CONTIGUOUS) ERR;
    if (nc_get_var_int(ncid, varid, data_in)) ERR;
    for (i = 0; i < DIM6_LEN; i++)
...
#define DIM17_2_LEN 1000
    ...
    /* Check stuff. */
    if (nc_inq(ncid, &ndims, &nvars, &natts, &unlimdimid)) ERR;
    if (ndims != NDIMS17 || nvars != 1 || natts != 0 || unlimdimid != -1) ERR;
    if (nc_inq_varids(ncid, &nvars, varids_in)) ERR;
    if (nvars != 1) ERR;
    if (varids_in[0] != 0) ERR;
    if (nc_inq_var(ncid, 0, name_in, &xtype_in, &ndims, dimids_in, &natts)) ERR;
    if (strcmp(name_in, VAR_NAME17) || xtype_in != NC_UINT64 || ndims != 2 || natts != 0 || dimids_in[0] != 0 || dimids_in[1] != 1) ERR;
    if (nc_inq_var_chunking(ncid, 0, &storage_in, chunksize_in)) ERR;
    for (d = 0; d < NDIMS17; d++)
      if (chunksize[d] != chunksize_in[d]) ERR;
    if (storage_in != NC_CHUNKED) ERR;
...
#define VAR_NAME10 "Jane_Porter"
  printf("**** testing that contiguous storage can't be turned on for vars with unlimited dims or filters...");
  {
    ...
    if (nc_def_var_deflate(ncid, varid2, 0, 1,
        NC_MIN_DEFLATE_LEVEL - 1)
        != NC_EINVAL) ERR;
    if (nc_def_var_deflate(ncid, varid2, 0, 1,
        NC_MAX_DEFLATE_LEVEL + 1)
        != NC_EINVAL) ERR;
