...
const char *path; /* name of writable netcdf file to open */
{
  ...
  if (ncattput(ncid, aa_units.var, aa_units.name,
      aa_units.type, aa_units.len, (void *)aa_units.val)
      == -1) {
    error("%s: ncattput failed", pname);
    ncclose(ncid);
    return ++nerrs;
...
const char *path; /* name of writable netcdf file to open */
{
  ...
  if ((jj_dim = ncdimdef(ncid, jj.name, jj.size)) == -1 || (kk_dim = ncdimdef(ncid, kk.name, kk.size)) == -1) {
    error("%s: ncdimdef failed", pname);
    ncclose(ncid);
    return ++nerrs;
  }
...
const char *path; /* name of writable netcdf file to open */
{
  ...
  for (iv = 0; iv < nv; iv++) {
    ...
    if (ncattput(ncid, cc_id, cc_units.name, cc_units.type,
        cc_units.len, (void *)cc_units.val)
        == -1) {
      error("%s: ncattput failed", pname);
      ncclose(ncid);
      return ++nerrs;
...
const char *path; /* name of writable netcdf file to open */
{
  ...
  if (ncattput(ncid0, dd_id, dd_fill_val.name, dd_fill_val.type,
      dd_fill_val.len, (void *)dd_fill_val.val)
      == -1) {
    error("%s: ncattput failed", pname);
    ncclose(ncid0);
    return ++nerrs;
