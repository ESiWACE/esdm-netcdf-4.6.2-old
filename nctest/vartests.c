...
const char *path; /* name of writable netcdf file to open */
{
  ...
  if ((varid = ncvardef(cdfid,
       xx.name, xx.type, xx.ndims, xx.dims))
      == -1) {
...
const char *path; /* name of writable netcdf file to open */
{
  ...
  if (ncvarput1(cdfid, -1, elm.coords, (void *)&elm.val) != -1 || ncvarput1(cdfid, test.nvars, elm.coords, (void *)&elm.val) != -1) {
...
const char *path; /* name of writable netcdf file to open */
{
  ...
  /* try with bad variable handle, should fail */
  if (ncvarget1(cdfid, -1, elm.coords, (void *)&elm.val) != -1 || ncvarget1(cdfid, test.nvars, elm.coords, (void *)&elm.val) != -1) {
...
const char *path; /* name of writable netcdf file to open */
{
  ...
  if ((yy_id = ncvardef(cdfid,
       yy.name, yy.type, yy.ndims, yy.dims))
      == -1) {
    error("%s: ncvardef failed", pname);
    ncclose(cdfid);
    return ++nerrs;
  ...
  if (ncvarinq(cdfid, yy_id, var.name,
      &var.type, &var.ndims, var.dims, &var.natts)
      == -1) {
