...
const char *path; /* name of writable netcdf file to open */
{
  ...
  if ((ww_id = ncvardef(cdfid,
       ww.name, ww.type, ww.ndims, ww.dims))
      == -1) {
...
const char *path; /* name of writable netcdf file to open */
{
  ...
  if ((uu_id = ncvardef(cdfid,
       uu.name, uu.type, uu.ndims, uu.dims))
      == -1) {
    error("%s: ncvardef failed", pname);
    ncclose(cdfid);
    return ++nerrs;
