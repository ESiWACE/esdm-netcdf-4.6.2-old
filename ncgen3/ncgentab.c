...
void equalatt() {
  ...
  for (i = 0; i < natts; i++) { /* expensive */
    if (atts[i].var == varnum && STREQ(atts[i].name, atts[natts].name)) {
...
int ncgwrap(void) /* returns 1 on EOF if no more input */
{
  return 1;
}
...
void clearout(void) /* reset symbol table to empty */
{
  YYSTYPE sp, tp;
