    ...
    switch (sym->subclass) {
      ...
      case NC_COMPOUND: /* keep if all fields are primitive*/
        ...
        for (j = 0; j < listlength(sym->subnodes); j++) {
          ...
          if (field->typ.basetype->subclass != NC_PRIM) {
            keep = 0;
            break;
          }
        }
...
inferattributetype1(Datalist *adata) {
  ...
  for (i = 0; i < datalistlen(adata); i++) {
    NCConstant *con = datalistith(adata, i);
    if (con->nctype == NC_FLOAT)
      forcefloat = 1;
    else if (con->nctype == NC_DOUBLE) {
      forcedouble = 1;
      break;
    }
  }
  ...
  for (i = 0; i < datalistlen(adata); i++) {
    ...
    if (con->value.uint64v > NC_MAX_INT64) {
      forceuint64 = 1;
      break;
    }
  }
  ...
  for (i = 0; i < datalistlen(adata); i++) {
    ...
    switch (con->nctype) {
      case NC_BYTE:
        if (con->value.int8v < 0) {
          hasneg = 1;
        }
        break;
      case NC_SHORT:
        if (con->value.int16v < 0) {
          hasneg = 1;
        }
        break;
      case NC_INT:
        if (con->value.int32v < 0) {
          hasneg = 1;
        }
        break;
    }
