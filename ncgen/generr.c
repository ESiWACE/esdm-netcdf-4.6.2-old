 ...
 */
void vderror(const char *fmt, va_list argv) {
 ...
 * Designed to be called by other vararg procedures
 */
void vdwarn(const char *fmt, va_list argv) {
...
void semwarn(const int lno, const char *fmt, ...) {
  va_list argv;
  va_start(argv, fmt);
...
void semerror(const int lno, const char *fmt, ...) {
  va_list argv;
  va_start(argv, fmt);
  (void)fprintf(stderr, "%s: %s line %d: ", progname, cdlname, lno);
  vderror(fmt, argv);
...
void nprintf(char *buffer, size_t size, const char *fmt, ...) {
  long written;
  va_list args;
  va_start(args, fmt);
