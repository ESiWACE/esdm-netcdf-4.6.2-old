...
/**************************************************/
#ifdef GENDEBUG
void report(char *lead, Datalist *list) {
...
/* Deep constant clear*/
void clearconstant(NCConstant *con) {
  if (con == NULL) return;
  switch (con->nctype) {
    case NC_STRING:
...
void codedump(Bytebuffer *buf) {
  bbCatbuf(codebuffer, buf);
  bbClear(buf);
}

void codepartial(const char *txt) {
...
void codelined(int n, const char *txt) {
...
void bbindent(Bytebuffer *buf, const int n) {
...
void bbprintf(Bytebuffer *buf, const char *fmt, ...) {
  va_list argv;
  va_start(argv, fmt);
...
void bbprintf0(Bytebuffer *buf, const char *fmt, ...) {
  va_list argv;
  va_start(argv, fmt);
  bbClear(buf);
  vbbprintf(buf, fmt, argv);
...
void dlappend(Datalist *dl, NCConstant *constant) {
  if (dl->length >= dl->alloc)
    dlextend(dl);
  dl->data[dl->length++] = (constant);
}

void dlset(Datalist *dl, size_t pos, NCConstant *constant) {
...
void reclaimconstant(NCConstant *con) {
  if (con == NULL) return;
  switch (con->nctype) {
    case NC_STRING:
      if (con->value.stringv.stringv != NULL)
...
void reclaimdatalist(Datalist *list) {
  int i;
  if (list == NULL) return;
  if (list->data != NULL) {
