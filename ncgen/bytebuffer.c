...
int bbSetalloc(Bytebuffer *bb, const unsigned int sz0) {
  unsigned int sz = sz0;
  char *newcontent;
...
void bbFree(Bytebuffer *bb) {
  if (bb == NULL) return;
  if (bb->content != NULL) efree(bb->content);
  efree(bb);
}

int bbSetlength(Bytebuffer *bb, const unsigned int sz) {
  if (bb == NULL) return bbFail();
  if (bb->length < sz) {
...
int bbFill(Bytebuffer *bb, const char fill) {
  unsigned int i;
  if (bb == NULL) return bbFail();
...
int bbGet(Bytebuffer *bb, unsigned int index) {
  if (bb == NULL) return -1;
  if (index >= bb->length) return -1;
  return bb->content[index];
}

int bbSet(Bytebuffer *bb, unsigned int index, char elem) {
  if (bb == NULL) return bbFail();
...
int bbAppend(Bytebuffer *bb, char elem) {
  if (bb == NULL) return bbFail();
...
int bbCat(Bytebuffer *bb, const char *s) {
...
int bbCatbuf(Bytebuffer *bb, const Bytebuffer *s) {
  if (bbLength(s) > 0)
...
int bbAppendn(Bytebuffer *bb, const void *elem, const unsigned int n0) {
  unsigned int n = n0;
  ...
  while (!bbNeed(bb, (n + 1))) {
    if (!bbSetalloc(bb, 0)) return bbFail();
  }
...
int bbInsert(Bytebuffer *bb, const unsigned int index, const char elem) {
  char tmp[2];
  tmp[0] = elem;
  return bbInsertn(bb, index, tmp, 1);
}

int bbInsertn(Bytebuffer *bb, const unsigned int index, const char *elem, const unsigned int n) {
  unsigned int i;
  int j;
...
int bbTailpop(Bytebuffer *bb, char *pelem) {
  if (bb == NULL) return bbFail();
  if (bb->length == 0) return bbFail();
...
int bbHeadpeek(Bytebuffer *bb, char *pelem) {
  if (bb == NULL) return bbFail();
  if (bb->length == 0) return bbFail();
  *pelem = bb->content[0];
  return TRUE;
...
int bbTailpeek(Bytebuffer *bb, char *pelem) {
  if (bb == NULL) return bbFail();
  if (bb->length == 0) return bbFail();
  *pelem = bb->content[bb->length - 1];
  return TRUE;
...
int bbSetcontents(Bytebuffer *bb, char *contents, const unsigned int alloc) {
  if (bb == NULL) return bbFail();
  bbClear(bb);
