...
void cquotestring(Bytebuffer *databuf, char quote) {
...
xescapifychar(unsigned int c, int quote, Bytebuffer *s) {
  if (c >= ' ' && c < '\177') {
    char *p;
    char **q;
    for (p = printescapable, q = printescape; *p; p++, q++) {
      if (c == *p) break;
    }
    if (*p) {
      bbAppend(s, '&');
  ...
  for (i = 0; i < slen; i++) {
    ...
    if (i > 0) {
      if (!lastcharescaped && thischarescaped)
        bbAppend(databuf, '\'');
      else if (lastcharescaped && !thischarescaped)
        bbCat(databuf, "//'");
