...
int ncsize(nc_type nctype) {
  if (nctype >= NC_NAT && nctype <= NC_COMPOUND)
    return ncsizes[nctype];
  return 0;
}

int hasunlimited(Dimset *dimset) {
  int i;
...
int isunlimited0(Dimset *dimset) {
...
int unsignedtype(nc_type nctype) {
  switch (nctype) {
    case NC_UBYTE:
    case NC_USHORT:
    case NC_UINT:
...
int isfloattype(nc_type nctype) {
  return (nctype == NC_FLOAT || nctype <= NC_DOUBLE);
}

int isclassicprim(nc_type nctype) {
  return (nctype >= NC_BYTE && nctype <= NC_DOUBLE);
}

int isclassicprimplus(nc_type nctype) {
  return (nctype >= NC_BYTE && nctype <= NC_DOUBLE)
         || (nctype == NC_STRING);
}

int isprim(nc_type nctype) {
  return (nctype >= NC_BYTE && nctype <= NC_STRING);
}

int isprimplus(nc_type nctype) {
  return (nctype >= NC_BYTE && nctype <= NC_STRING)
         || (nctype == NC_ECONST)
         || (nctype == NC_OPAQUE);
}

void collectpath(Symbol *grp, List *grpstack) {
  while (grp != NULL) {
...
prefixdup(List *prefix) {
  ...
  for (i = 0; i < listlength(prefix); i++)
    listpush(dupseq, listget(prefix, i));
  return dupseq;
}
...
*/
int findunlimited(Dimset *dimset, int start) {
...
If no unlimited exists, return |dimset|
*/
int findlastunlimited(Dimset *dimset) {
  int i;
...
*/
int countunlimited(Dimset *dimset) {
  int i, count;
