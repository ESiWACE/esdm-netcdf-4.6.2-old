...
int main() {
  ...
  printf("**** Expecting NC_EACCESS when adding _FillValue attribute if variable exists.\n");
  status = nc_put_att_int(ncid, test_id, "_FillValue", NC_INT, 1, test_fill_val);
  // if (status != NC_EACCESS) {
  //     fflush(stdout); /* Make sure our stdout is synced with stderr. */
  //     err++;
  //     fprintf(stderr, "Sorry! Expecting NC_EACCESS but got %s, at file %s line: %d\n",
