...
int main(int argc, char **argv) {
  ...
  printf("*** testing simple opens and creates...");
  {
    ...
    if (ndims != 1 || strcmp(var_name, VAR1_NAME) || var_type != NC_BYTE || dimids_var[0] != dimids[0] || natts != 0) ERR;
    if (nc_close(ncid)) ERR;
    ...
    if (dim_len != DIM1_LEN || strcmp(dim_name, DIM1_NAME)) ERR;
    if (nc_inq_var(ncid, 0, var_name, &var_type, &ndims, dimids_var, &natts)) ERR;
    if (ndims != 1 || strcmp(var_name, VAR1_NAME) || var_type != NC_INT || dimids_var[0] != dimids[0] || natts != 0) ERR;
    if (nc_close(ncid)) ERR;
  }
  SUMMARIZE_ERR;
  printf("*** testing more complex opens and creates...");
  {
    ...
    if (nc_inq_var(ncid, 0, var_name, &var_type, &ndims, dimids_var, &natts)) ERR;
    if (ndims != 2 || strcmp(var_name, VAR1_NAME) || var_type != NC_INT || dimids_var[0] != dimids[0] || natts != 0) ERR;
    if (nc_inq_var(ncid, 1, var_name, &var_type, &ndims, dimids_var, &natts)) ERR;
    if (ndims != 2 || strcmp(var_name, VAR2_NAME) || var_type != NC_UINT || dimids_var[1] != dimids[1] || natts != 0) ERR;
...
int test_redef(int format) {
  ...
  if (cache_size_in != NEW_CACHE_SIZE || cache_nelems_in != NEW_CACHE_NELEMS || cache_preemption_in != NEW_CACHE_PREEMPTION) ERR;
  cache_size_in = 0;
  ...
  if (cache_size_int_in != NEW_CACHE_SIZE || cache_nelems_int_in != NEW_CACHE_NELEMS || cache_preemption_int_in != (int)(NEW_CACHE_PREEMPTION * 100)) ERR;
  ...
  if (nc_set_chunk_cache_ints(-1, NEW_CACHE_NELEMS_2,
      (int)(NEW_CACHE_PREEMPTION_2 * 100))
      != NC_EACCESS) ERR;
  ...
  if (strcmp(var_name, REDEF_VAR1_NAME) || xtype_in != NC_INT || ndims != REDEF_NDIMS || dimids_in[0] != dimids[0] || dimids_in[1] != dimids[1]) ERR;
  if (nc_inq_var(ncid, 1, var_name, &xtype_in, &ndims, dimids_in, &natts)) ERR;
  if (strcmp(var_name, REDEF_VAR2_NAME) || xtype_in != NC_BYTE || ndims != REDEF_NDIMS || dimids_in[0] != dimids[0] || dimids_in[1] != dimids[1]) ERR;
  ...
  if (ndims != REDEF_NDIMS || strcmp(var_name, REDEF_VAR3_NAME) || var_type != NC_BYTE || natts != 0) ERR;
  ...
  if (nc_inq_var(ncid, 0, var_name, &var_type, &ndims, dimids_var, &natts)) ERR;
  if (ndims != REDEF_NDIMS || strcmp(var_name, REDEF_VAR1_NAME) || var_type != NC_INT || natts != 0) ERR;
  if (nc_inq_var(ncid, 1, var_name, &var_type, &ndims, dimids_var, &natts)) ERR;
  if (ndims != REDEF_NDIMS || strcmp(var_name, REDEF_VAR2_NAME) || var_type != NC_BYTE || natts != 0) ERR;
  if (nc_inq_var(ncid, 2, var_name, &var_type, &ndims, dimids_var, &natts)) ERR;
  if (ndims != REDEF_NDIMS || strcmp(var_name, REDEF_VAR3_NAME) || var_type != NC_BYTE || natts != 0) ERR;
