...
int check_nc4_file(int ncid) {
  ...
  if (strncmp(norm_utf8, name_in, strlen(norm_utf8)) || size_in != 1 || base_type_in != NC_BYTE || nfields_in != 1 || class_in != NC_ENUM) ERR;
  name_in[0] = size_in = base_type_in = 0;
  if (nc_inq_enum(grpid2, enum_typeid, name_in, &base_type_in, &size_in, &num_mem)) ERR;
  if (strncmp(norm_utf8, name_in, strlen(norm_utf8)) || size_in != 1 || base_type_in != NC_BYTE || num_mem != 1) ERR;
  ...
  if (nc_inq_user_type(grpid3, comp_typeid, name_in, &size_in, &base_type_in,
      &nfields_in, &class_in)) ERR;
  if (strncmp(norm_utf8, name_in, sizeof(norm_utf8)) || size_in != sizeof(struct comp) || base_type_in != NC_NAT || nfields_in != 1 || class_in != NC_COMPOUND) ERR;
  size_in = nfields_in = 999;
  if (nc_inq_compound(grpid3, comp_typeid, name_in, &size_in, &nfields_in)) ERR;
  if (strncmp(norm_utf8, name_in, sizeof(norm_utf8)) || size_in != sizeof(struct comp) || nfields_in != 1) ERR;
  name_in[0] = 0;
...
/* Given an ncid, check the file to make sure it has all the objects I
 * expect. */
int check_classic_file(int ncid) {
...
int main(int argc, char **argv) {
  ...
  printf("*** creating UTF-8 names in classic model netcdf files...");
  {
    ...
    for (f = NC_EACCESS; f < NC_FORMAT_CLASSIC; f++) {
