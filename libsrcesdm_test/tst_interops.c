...
int main(int argc, char **argv) {
  ...
  printf("*** Checking that HDF5 file can be read by netCDF-4, and adding an att...");
  {
    ...
    if (strcmp(name_in, PRES_NAME) || xtype_in != NC_FLOAT || ndims_in != 2 || natts_in != 0) ERR;
...

  {
    ...
    /* Close up the shop. */
    if (H5Tclose(typeid) < 0 || H5Aclose(attid) < 0 || H5Sclose(spaceid) < 0 || H5Dclose(datasetid) < 0 || H5Gclose(grpid) < 0 || H5Fclose(fileid) < 0) ERR;
  }
  SUMMARIZE_ERR;
  ...
  printf("*** Creating a HDF5 file with two vars and no dimension scales, without creation ordering...");
  {
    ...
    /* Ring down the curtain. */
    if (H5Dclose(pres_datasetid) < 0 || H5Dclose(temp_datasetid) < 0 || H5Sclose(spaceid) < 0 || H5Fclose(fileid) < 0) ERR;
