...
int create_test_file(char *filename, int *varid, int *ncid) {
  int dimid;
  int type;
...
#define NUM_ENHANCED_TYPES 12
  ...
  printf("*** testing user-defined netCDF-4 enum types and equality...");
  {
    ...
    if (nc_def_compound(ncid4, TEST_VAL_42, A_NAME, &typeid3) != NC_NOERR) ERR;
    if (nc_def_vlen(ncid4, A_NAME, NC_INT, &typeid3) != NC_NOERR) ERR;
  ...
  printf("*** testing vara functions with new netCDF-4 atomic types...");
  {
    ...
    /* This will not work. */
    if (nc_put_vara_uchar(ncid, varid[0], NULL, count, ubyte_data_out) != NC_EINVALCOORDS) ERR;

    /* This will work. */
    if (nc_put_vara_uchar(ncid, varid[0], start, count, ubyte_data_out)) ERR;

    /* This will not work. */
    if (nc_get_vara_uchar(ncid, varid[0], NULL, count, ubyte_data_in) != NC_EINVALCOORDS) ERR;
  ...
  printf("*** testing var1 functions with new netCDF-4 atomic types...");
  {
    ...
    /* This will not work. */
    if (nc_put_var1_uchar(ncid, varid[0], NULL, ubyte_data_out) != NC_EINVALCOORDS) ERR;
