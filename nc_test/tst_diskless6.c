...
void notexists(const char *file) {
  FILE *f = fopen(file, "r");
  if (f != NULL) {
    fclose(f);
    fail(NC_EEXIST);
...
#define TESTCREATE(file, mode, exist) \
  {                                   \
    lineno = __LINE__;                \
    testcreate(file, mode, exist);    \
  }
#define TESTOPEN(file, mode, rw) \
  {                              \
    lineno = __LINE__;           \
    testopen(file, mode, rw);    \
  }
...
testopen(const char *file, int mode, int rdwrite) {
  ...
  ret = nc_close(ncid);
  if (ret != NC_NOERR && rdwrite)
    fail(ret);
  else if (ret == NC_NOERR && !rdwrite)
    fail(NC_EINVAL);
