...
int main(int argc, char **argv) {
  ...
  if (argc == 2)
    ...
    strcpy(filename, "tst_def_var_fill.nc");
  ...
  for (k = 0; k < 5; k++) {
...
#endif
#ifndef USE_NETCDF4
    if (formats[k] == NC_FORMAT_NETCDF4 || formats[k] == NC_FORMAT_NETCDF4_CLASSIC)
      continue;
#endif
    ...
    /* define dimension */
    err = nc_def_dim(ncid, "Y", NY, &dimid[0]);
    CHECK_ERR
    err = nc_def_dim(ncid, "X", NX, &dimid[1]);
    CHECK_ERR

    /* define variables */
    err = nc_def_var(ncid, "var_nofill", NC_INT, 2, dimid, &varid[0]);
    CHECK_ERR
    err = nc_def_var(ncid, "var_fill", NC_INT, 2, dimid, &varid[1]);
    CHECK_ERR

    /* set fill mode for variables */
    err = nc_def_var_fill(ncid, NC_GLOBAL, 0, NULL);
    EXP_ERR(NC_EGLOBAL)
    err = nc_def_var_fill(ncid, varid[0], 1, NULL);
    CHECK_ERR
    err = nc_def_var_fill(ncid, varid[1], 0, NULL);
    CHECK_ERR
    ...
    err = nc_open(filename, NC_WRITE, &ncid);
    CHECK_ERR

    /* get variable IDs */
    err = nc_inq_varid(ncid, "var_nofill", &varid[0]);
    CHECK_ERR
    err = nc_inq_varid(ncid, "var_fill", &varid[1]);
    CHECK_ERR

    /* read variable "var_nofill" and check contents */
    for (i = 0; i < NY * NX; i++)
      buf[i] = -1;
    err = nc_get_var_int(ncid, varid[0], buf);
    CHECK_ERR
    for (i = 0; i < NY; i++) {
      for (j = 0; j < NX; j++) {
        if (2 <= j && j < 4) {
    ...
    for (i = 0; i < NY * NX; i++)
      buf[i] = -1;
    err = nc_get_var_int(ncid, varid[1], buf);
    CHECK_ERR
    for (i = 0; i < NY; i++) {
      for (j = 0; j < NX; j++) {
