...
#define ERRORI                                                                 \
  {                                                                            \
    printf("Error at line %d (loop=%d): %s\n", __LINE__, i, nc_strerror(res)); \
    nerrs++;                                                                   \
    continue;                                                                  \
  }
...
#define MAX_ATTSTRING_LEN 100
  ...
  for (j = 0; j < num_formats; j++) {
	...
	* bad names and make sure these are rejected */
    ...
    for (i = 0; i < NUM_GOOD; i++) {
      size_t attlen;
      if ((res = nc_inq_dimid(ncid, valid[i], &dimid)) || dimid != dimids[i])
        ERRORI
      if ((res = nc_inq_varid(ncid, valid[i], &varid)) || varid != varids[i])
        ERRORI
