 ...
 */
int inRange3(const int cdf_format,
const double value,
const nc_type xtype,
    ...
    switch (index[0]) {
      ...
      case 1:
        switch (xtype) {
          ...
          case NC_INT64:
            return X_INT_MAX + 128.0;
            /* slightly bigger than INT_MAX */
          case NC_UINT64:
            return X_UINT_MAX + 128.0;
...
const nct_itype itype) {
  ...
  /* netCDF specification make a special case for type conversion between
     ...
     * and unsigned char.
     */
  if (cdf_format < NC_FORMAT_CDF5 && itype == NCT_UCHAR && xtype == NC_BYTE && result >= -128 && result < 0)
    result += 256;
...
int file_create(const char *filename, int cmode, int *ncid) {
  ...
  if (default_format == NC_FORMAT_CLASSIC || default_format == NC_FORMAT_64BIT_OFFSET || default_format == NC_FORMAT_64BIT_DATA)
...
int test_nc_against_pnetcdf(void) {
  ...
  IF(err != NC_NOERR)
  error("nc_open_par: %s", nc_strerror(err));
  check_dims(ncid);
  check_vars(ncid);
  check_atts(ncid);
  err = nc_close(ncid);
  ...
  IF(err != NC_NOERR)
  error("nc_create_par: %s", nc_strerror(err));
  def_dims(ncid);
  def_vars(ncid);
  put_atts(ncid);
  err = nc_enddef(ncid);
