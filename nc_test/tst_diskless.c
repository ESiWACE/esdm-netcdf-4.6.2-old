...
int main(int argc, char **argv) {
  ...
  if (mmap && diskless) {
    fprintf(stderr, "Illegal: mmap+diskless\n");
    exit(1);
  };
  if (mmap && usenetcdf4) {
    fprintf(stderr, "Illegal: mmap+netcdf4\n");
    exit(1);
  };
  ...
  printf("*** testing diskless file with scalar vars...");
  {
    ...
    if (strcmp(name_in, RESISTOR) || type_in != NC_INT || ndims_in != 0 || natts_in != 0) ERR;
    if (nc_inq_var(ncid, varid1, name_in, &type_in, &ndims_in, NULL, &natts_in)) ERR;
    if (strcmp(name_in, CAPACITOR) || type_in != NC_FLOAT || ndims_in != 0 || natts_in != 0) ERR;
...
#define VAR2_NAME "taxi_distance"
    ...
    * of scientists to understand my data. */
    ...
    if (strcmp(name_in, VAR0_NAME) || type_in != NC_INT || ndims_in != NDIMS || dimid_in[0] != 0 || dimid_in[1] != 1 || natts_in != 0) ERR;
    if (nc_inq_var(ncid, varid1, name_in, &type_in, &ndims_in, dimid_in, &natts_in)) ERR;
    if (strcmp(name_in, VAR1_NAME) || type_in != NC_FLOAT || ndims_in != 0 || natts_in != 0) ERR;
    if (nc_inq_var(ncid, varid2, name_in, &type_in, &ndims_in, dimid_in, &natts_in)) ERR;
    if (strcmp(name_in, VAR2_NAME) || type_in != NC_SHORT || ndims_in != 1 || dimid_in[0] != 1 || natts_in != 0) ERR;
...
#define STAR_WARS "number_of_555_timer_chips"
    ...
    /* Check variables. */
    if (nc_inq_var(ncid, varid0, name_in, &type_in, &ndims_in, NULL, &natts_in)) ERR;
    if (strcmp(name_in, DUNE) || type_in != NC_INT || ndims_in != 0 || natts_in != 0) ERR;
    if (nc_inq_var(ncid, varid1, name_in, &type_in, &ndims_in, NULL, &natts_in)) ERR;
