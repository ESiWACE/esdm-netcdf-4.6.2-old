...
#define CHECK(expr)                              \
  {                                              \
    stat = check((expr), __FILE__, __LINE__, 0); \
    if (stat) return stat;                       \
  }
#define XCHECK(expr)                             \
  {                                              \
    stat = check((expr), __FILE__, __LINE__, 1); \
    if (stat) return stat;                       \
  }

#define REPORT(xfail, expr) \
  {                         \
    if ((xfail)) {          \
      XCHECK((expr));       \
    } else {                \
...
*/
int duplicatememory(NC_memio *src, NC_memio *target, size_t alloc) {
...
verify_file(int ncid, int modified) {
  ...
  if (strcmp(name_in, VAR0_NAME) || type_in != NC_INT || ndims_in != NDIMS || dimid_in[0] != 0 || dimid_in[1] != 1 || natts_in != 0) CHECK(NC_EINVAL);
  CHECK(nc_inq_var(ncid, varid[1], name_in, &type_in, &ndims_in, dimid_in, &natts_in));
  if (strcmp(name_in, VAR1_NAME) || type_in != NC_FLOAT || ndims_in != 0 || natts_in != 0) CHECK(NC_EINVAL);
  CHECK(nc_inq_var(ncid, varid[2], name_in, &type_in, &ndims_in, dimid_in, &natts_in));
  if (strcmp(name_in, VAR2_NAME) || type_in != NC_SHORT || ndims_in != 1 || dimid_in[0] != 1 || natts_in != 0) CHECK(NC_EINVAL);
  ...
  if (modified) {
    ...
    if (strcmp(name_in, VAR3_NAME) || type_in != NC_INT || ndims_in != 1 || dimid_in[0] != 0 || natts_in != 0) CHECK(NC_EINVAL);
