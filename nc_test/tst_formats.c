...
int check_inq_format(int ncid, int expected_format, int expected_extended_format, int expected_mode) {
  int format;
  int extended_format;
  int mode;
  ...
  /* Nothing to do with inq_format, but let's check the base_pe
    * functions. */
  if (expected_format == NC_FORMAT_CLASSIC || expected_format == NC_FORMAT_64BIT_OFFSET || expected_format == NC_FORMAT_CDF5) {
    ...
    for (f = 0; f < num_formats; f++) {
      ...
      for (d = 0; d < NUM_FILL_WRITE_TESTS; d++) {
        ...
        for (a = 0; a < NUM_FILL_WRITE_METHOD_TESTS; a++) {
          ...
          if (format[f] == NC_FORMAT_CLASSIC || format[f] == NC_FORMAT_64BIT_OFFSET || format[f] == NC_FORMAT_CDF5) {
            if (ret) ERR;
          } else {
