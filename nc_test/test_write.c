...
nc_get_file_version(char *path, int *version) {
  ...
  if (strncmp(magic, "CDF", MAGIC_NUM_LEN - 1) == 0) {
    if (magic[MAGIC_NUM_LEN - 1] == NC_FORMAT_CLASSIC || magic[MAGIC_NUM_LEN - 1] == NC_FORMAT_64BIT_OFFSET
#ifdef ENABLE_CDF5
 ...
 *    close file & create again for test using attribute _FillValue
 */
int test_nc_set_default_format(void) {
  int ncid, nok = 0;
  int err;
  int i;
  int version = 1;
