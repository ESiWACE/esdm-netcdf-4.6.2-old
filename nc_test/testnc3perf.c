 ...
 */
void
test_slabs(ncid, sizes) int ncid; /* handle of netcdf open and in data mode */
          ...
          for (ii = corner[idim]; ii < edge[idim]; ii++) {
            for (jj = corner[jdim]; jj < edge[jdim]; jj++) {
              for (kk = corner[kdim]; kk < edge[kdim]; kk++) {
                ...
                if (val_diff(va[iv].type, v,
                    ((ii) * (int)edge[jdim] + jj) * (int)edge[kdim] + kk, VF(point))) {
                  fprintf(stderr,
