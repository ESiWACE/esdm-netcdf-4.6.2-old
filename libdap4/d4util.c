...
int ncd4__testurl(const char *path, char **basenamep) {
  NCURI *uri;
  int ok = NC_NOERR;
...
NCD4_typesize(nc_type tid) {
  switch (tid) {
    ...
    case NC_SHORT:
    case NC_USHORT: return sizeof(short);
    case NC_INT:
    case NC_UINT: return sizeof(int);
    case NC_FLOAT: return sizeof(float);
    case NC_DOUBLE: return sizeof(double);
    case NC_INT64:
    case NC_UINT64: return sizeof(long long);
...
int NCD4_parseFQN(const char *fqn0, NClist *pieces) {
  int ret = NC_NOERR;
  int count;
  char *p;
  char *start;
...
int NCD4_mktmp(const char *base, char **tmpnamep) {
  int fd;
...
void NCD4_hostport(NCURI *uri, char *space, size_t len) {
  if (space != NULL && len > 0) {
...
void NCD4_userpwd(NCURI *uri, char *space, size_t len) {
  if (space != NULL && len > 0) {
    space[0] = '\0'; /* so we can use strncat */
...
int NCD4_error(int code, const int line, const char *file, const char *fmt, ...) {
  va_list argv;
...
int NCD4_errorNC(int code, const int line, const char *file) {
