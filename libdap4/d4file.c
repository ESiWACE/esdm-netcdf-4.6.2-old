...
void *mpidata, NC_Dispatch *dispatch, NC *nc) {
  ...
  if ((d4info->substrate.metadata = NCD4_newmeta(ncbyteslength(d4info->curl->packet),
       ncbytescontents(d4info->curl->packet)))
      == NULL) {
    ret = NC_ENOMEM;
    goto done;
  }
...
int NCD4_close(int ncid, void *ignore) {
  int ret = NC_NOERR;
  NC *nc;
  NCD4INFO *d4info;
  int substrateid;
...
set_curl_properties(NCD4INFO *d4info) {
  ...
  if (d4info->auth.curlflags.cookiejar == NULL) {
    ...
    len = strlen(ncrc_globalstate.tempdir)
          + 1 /* '/' */
          + strlen("ncd4cookies");
...
void NCD4_applyclientparamcontrols(NCD4INFO *info) {
  const char *value;
