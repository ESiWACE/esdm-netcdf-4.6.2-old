...
int NCD4_metabuild(NCD4meta *metadata, int ncid) {
  int ret = NC_NOERR;
  int i;
...
void NCD4_setdebuglevel(NCD4meta *meta, int debuglevel) {
  meta->debuglevel = debuglevel;
}

void NCD4_reclaimMeta(NCD4meta *dataset) {
  int i;
...
void reclaimNode(NCD4node *node) {
  if (node == NULL) return;
  nullfree(node->name);
...
copyAtomic(union ATOMICS *converter, nc_type type, size_t len, void *dst, NClist *blobs) {
  switch (type) {
    ...
    case NC_BYTE:
    case NC_UBYTE:
      memcpy(dst, &converter->u8[0], len);
      break;
    case NC_SHORT:
    case NC_USHORT:
      memcpy(dst, &converter->u16[0], len);
      break;
    case NC_INT:
    case NC_UINT:
      memcpy(dst, &converter->u32[0], len);
      break;
    case NC_INT64:
    case NC_UINT64:
      memcpy(dst, &converter->u64[0], len);
      break;
    case NC_FLOAT:
      memcpy(dst, &converter->f32[0], len);
      break;
    case NC_DOUBLE:
      memcpy(dst, &converter->f64[0], len);
      break;
    case NC_STRING:
...
decodeEconst(NCD4meta *builder, NCD4node *enumtype, const char *nameorval, union ATOMICS *converter) {
  ...
  if (!match) {
    ...
    for (i = 0; i < nclistlength(enumtype->en.econsts); i++) {
      NCD4node *ec = (NCD4node *)nclistget(enumtype->en.econsts, i);
      if (ec->en.ecvalue.u64[0] == number.u64[0]) {
        match = ec;
        break;
      }
    }
