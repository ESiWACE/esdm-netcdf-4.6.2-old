...
int dcemergeprojections(DCEprojection *merged, DCEprojection *addition) {
  int ncstat = NC_NOERR;
  int i, j;
      ...
      *clone = *orig;
      switch (clone->discrim) {
        ...
        case CES_VAR:
          ...
          break;
        case CES_FCN:
          clone->fcn = (DCEfcn *)dceclone((DCEnode *)orig->fcn);
          break;
        default: assert(0);
      }
    } break;
...
void dcetobuffer(DCEnode *node, NCbytes *buf) {
  dcedump(node, buf);
}
...
void dcelisttobuffer(NClist *list, NCbytes *buf, char *sep) {
  int i;
...
int dceiswholeslice(DCEslice *slice) {
  if (slice->first != 0
      || slice->stride != 1
...
int dcesamepath(NClist *list1, NClist *list2) {
  int i;
...
void dcesegment_transpose(DCEsegment *segment,
size_t *start,
size_t *count,
size_t *stride,
size_t *sizes) {
