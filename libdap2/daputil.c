...
/* Pad a buffer */
int dapalignbuffer(NCbytes *buf, int alignment) {
  int pad;
  unsigned long len;
...
*/
int dapparamcheck(NCDAPCOMMON *nccomm, const char *key, const char *subkey) {
  const char *value;
  char *p;
...
int nclistdeleteall(NClist *l, void *elem) {
...
void collectnodepath(CDFnode *node, NClist *path, int withdataset) {
  if (node == NULL) return;
...
void collectocpath(OClink conn, OCddsnode node, NClist *path) {
  OCddsnode container;
  OCtype octype;
  if (node == NULL) return;
...
void clonenodenamepath(CDFnode *node, NClist *path, int withdataset) {
  if (node == NULL) return;
...
BOOL dapinsequence(CDFnode *node) {
...
BOOL dapinstructarray(CDFnode *node) {
...
getlimitnumber(const char *limit) {
  ...
  switch (limit[slen - 1]) {
    case 'G':
    case 'g': multiplier = GIGBYTE; break;
    case 'M':
    case 'm': multiplier = MEGBYTE; break;
    case 'K':
