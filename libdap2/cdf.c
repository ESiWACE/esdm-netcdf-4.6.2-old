    ...
    for (i = 0; i < nclistlength(varnodes); i++) {
      ...
      for (j = 0; j < i; j++) {
        ...
        if (strcmp(var->ncfullname, testnode->ncfullname) != 0)
          ...
          for (d = 0; d < nclistlength(testnode->array.dimsetall); d++) {
...
*/
int nodematch(CDFnode *node1, CDFnode *node2) {
...
int simplenodematch(CDFnode *node1, CDFnode *node2) {
  ...
  if (node1->nctype != node2->nctype) {
    ...
    int structgrid = ((node1->nctype == NC_Grid && node2->nctype == NC_Structure)
                      || (node1->nctype == NC_Structure && node2->nctype == NC_Grid)
                      ? 1
                      : 0);
    if (!structgrid)
      return 0;
