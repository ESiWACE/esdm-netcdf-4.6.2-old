...
int dapiswholeslice(DCEslice *slice, CDFnode *dim) {
...
int dapiswholesegment(DCEsegment *seg) {
  int i, whole;
  NClist *dimset = NULL;
  unsigned int rank;
...
int dapiswholeprojection(DCEprojection *proj) {
  int i, whole;
...
int dapiswholeconstraint(DCEconstraint *con) {
  int i;
...
*/
int daprestrictprojection(NClist *projections, DCEprojection *var, DCEprojection **resultp) {
  int ncstat = NC_NOERR;
  int i;
