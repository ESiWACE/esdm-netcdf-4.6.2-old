...
*/
int iscached(NCDAPCOMMON *nccomm, CDFnode *target, NCcachenode **cachenodep) {
  int i, j, found, index;
  NCcache *cache;
...
void freenccachenode(NCDAPCOMMON *nccomm, NCcachenode *node) {
  if (node == NULL) return;
#ifdef DEBUG
...
void freenccache(NCDAPCOMMON *nccomm, NCcache *cache) {
  int i;
